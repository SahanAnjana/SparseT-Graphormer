...
def get_args_parser():
    parser = GMAEParser(
        "GMAE fine-tuning for forecasting", add_help=False
    )
    parser.add_argument(
        "--accum_iter",
        default=1,
...
def main(args):
    ...
    if not args.project_name:
        args.project_name = f'gmae_st_finetune_{args.task}_{args.dataset_name}_{args.model}'
    ...
    args.pred_node_dim = 1  # we only care about sensor values, not time of day
    ...
    # define the model
    model = model_finetune.__dict__[args.model](
        **vars(args),
    )
    model.to(device)
    if args.use_compile:
    ...
    criterion = torch.nn.HuberLoss(args.loss_delta)
    min_loss = 0.0
